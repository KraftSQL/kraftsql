...
open class GenericSubexpressionCollector<E : Engine<E>> : SubexpressionCollector<E> {
    override fun getSubexpressions(expression: Expression<E, *>) =
        when (expression) {
            ...
            is GreaterThan<E> -> listOf(expression.left, expression.right)
            is GreaterOrEqual<E> -> listOf(expression.left, expression.right)
            ...
            is Min<E, *> -> listOf(expression.expression)
            is Max<E, *> -> listOf(expression.expression)
            is SubqueryExpression<E, *> -> (expression.subquery.columns ?: emptyList() ).map { it.value }
